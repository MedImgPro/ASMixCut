...
os.environ["CUDA_VISIBLE_DEVICES"] = "4,6,7" # 0, 1, 2, ...
...
def mixup_process(out,
    ...
    if hidden:
        ...
        if box:
            # CutMix
            out,out1, out2, ratio = mixup_box(out, out[indices],out1, out1[indices],out2, out2[indices], alpha=lam, device='cuda')
            ...
            if block_num > 1:
                out, ratio = mixup_graph(out,
                                         ...
                                         adv_mask2=adv_mask2,
                                         mp=mp,
                                         device=target_reweighted.device)
            else:
                ratio = torch.ones(out.shape[0], device=target_reweighted.device)
        else:
            # Input Mixup

            out = out * lam + out[indices] * (1 - lam)
            ratio = torch.ones(out.shape[0], device=target_reweighted.device) * lam

    target_shuffled_onehot = target_reweighted[indices]
    target_reweighted = target_reweighted * ratio.unsqueeze(-1) + target_shuffled_onehot * (
        1 - ratio.unsqueeze(-1))

    return out, out1, out2, target_reweighted
...
def graphcut_multi(unary1, unary2, pw_x, pw_y, alpha, beta, eta, n_labels=2, eps=1e-8):
    ...
    # print('unary_cost',unary_cost)
    # print('pairwise_cost', pairwise_cost)
...
def mixup_box(input1, input2,input3, input4,input5, input6, alpha=0.5, device='cuda'):
    '''CutMix'''
    ...
    input1[:, :, x1:x2, y1:y2] = input1[:, :, x1:x2, y1:y2] * alpha + input2[:, :, x1:x2, y1:y2] * (1 - alpha)
    input3[:, :, x1:x2, y1:y2] = input3[:, :, x1:x2, y1:y2] * alpha + input4[:, :, x1:x2, y1:y2] * (1 - alpha)
    input5[:, :, x1:x2, y1:y2] = input5[:, :, x1:x2, y1:y2] * alpha + input6[:, :, x1:x2, y1:y2] * (1 - alpha)

    ratio += 1 - ((x2 - x1) * (y2 - y1) / (width * height))* alpha
    ratio = torch.tensor(ratio, dtype=torch.float32)

    if device == 'cuda':
        ratio = ratio.cuda()

    return input1,input3, input5, ratio
