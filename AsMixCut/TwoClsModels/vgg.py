...
def vgg11_bn(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        model.features._modules['0']=nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1,bias=False)
        model.classifier._modules['0'] = nn.Sequential(nn.Linear(25088*3, 4096))
        model.classifier._modules['6'] = nn.Sequential(nn.Linear(4096, 2))
    return model
...
def vgg19(pretrained=False, **kwargs):
    ...
    if pretrained:
        # #model.classifier._modules['0']
        #
        # vgg19 = eval('models.%s(init_weights=False)' % 'vgg19')
        # vgg19.load_state_dict(torch.load(models_dir))
        # vgg19.classifier._modules['6'] = nn.Sequential(nn.Linear(4096, 2), nn.Softmax(dim=1))
        model.load_state_dict(torch.load(os.path.join(models_dir, model_name['vgg19'])))
       # model.classifier._modules['6'] = nn.Sequential(nn.Linear(4096, 2), nn.Sigmoid())
