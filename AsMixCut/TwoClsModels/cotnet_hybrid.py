...
from TwoClsModels.helpers import build_model_with_cfg
from TwoClsModels.layers import SelectiveKernelConv, ConvBnAct, create_attn
from TwoClsModels.registry import register_model
from TwoClsModels.resnet import ResNet
from TwoClsModels.layers import Shiftlution
from TwoClsModels.cupy_layers.aggregation_zeropad import LocalConvolution
from TwoClsModels.layers import create_act_layer
import torch.nn.functional as F
from torch import einsum
from TwoClsModels.layers.activations import Swish
from TwoClsModels.layers.tbconv import TBConv
from TwoClsModels.layers.kerv2d import Kerv2d
from TwoClsModels.layers import get_act_layer
from TwoClsModels.layers import SplitAttnConv2d
from TwoClsModels.layers import DropBlock2d, DropPath, AvgPool2dSame, BlurPool2d, create_classifier
...
class CoTHybridNet(nn.Module):
    ...
    def forward(self, x1, x2, x3):
        ...
        if self.drop_rate:
            x1 = F.dropout(x1, p=float(self.drop_rate), training=self.training)

        x2 = self.forward_features(x2)
        x2 = self.global_pool(x2)
        if self.drop_rate:
            x2 = F.dropout(x2, p=float(self.drop_rate), training=self.training)

        x3 = self.forward_features(x3)
        x3 = self.global_pool(x3)
        if self.drop_rate:
            x3 = F.dropout(x3, p=float(self.drop_rate), training=self.training)
...
def se_cotnetd_50(pretrained=False, **kwargs):
    ...
    model = _create_se_cotnetd('cot_basic', pretrained, **model_args)
    return model
